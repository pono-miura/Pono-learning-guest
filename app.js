...
function showQ(){if(qi>=unit.qs.length)return finish();let q=unit.qs[qi];A.innerHTML=`<div class="card"><span class="tag">③ 自分でやる</span><p>${qi+1}/${unit.qs.length}　確認：${q.skill}</p><h1>${q.q}</h1>${q.c.map((c,i)=>`<button class="light" onclick="answer(${i})">${c}</button>`).join("")}<button class="hint" onclick="showHint(${qi})">💡 ヒントを見る</button><button class="light" onclick="dontKnow()">わからない・説明を見る</button></div>`}
